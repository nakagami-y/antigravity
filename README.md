...
3. `projects/<project>/`
   - 実務は project 単位でここに入ります
   - `udemy` でも `unikoschool-launch` でも、まず対象 project の README を確認します
4. `memory_bank/plan/`
   - 人間がエージェントへ指示したら、開始前にここへ plan を残します
5. `memory_bank/report/`
   - 人間から受けた指示に対して、AI が何を見てどう判断したかを残す場所です
...
- `UdemyのX投稿を作って。plan と report も残して。目的は認知、CTAは講座ページを見る。`
- `Udemy講座のLINE告知文を作って。plan と report も残して。申込み促進が目的。`
- `Udemy投稿として3案出してから、最後に1本へまとめて。plan と report も残して。`

`Udemy投稿` や `UdemyのX投稿` のように書くと、Udemy workflow に寄せやすくなります。
人間側も、指示の時点で `plan と report も残して` を明示するのを標準にします。
...
1. root `memory_bank/plan/` に plan を作る
2. 対象 project の README と workflow を読む
3. 実作業を行う
4. root `memory_bank/report/` に report を作る

`/plan` や `/report` の手動指定がなくても、この流れを省略しません。

### ルールの最小形

- 人間がエージェントへ何か指示したら、内容の大小にかかわらず plan と report を残す
