...
> 注意: このディレクトリは講座台本・制作資料の保管層。AntiGravity の runtime 正本は `GEMINI.md` と `.agents/`、Udemy 投稿生成の正本は `projects/udemy/README.md` 以下です。セットアップ系台本の install / auth 手順は、配布前に必ず公式 docs で再確認すること。
...
### Sec 02: 準備（Google AntiGravity / VS Code / Git）

| 配布物 | ファイル | 説明 |
|--------|---------|------|
| Gitコマンド早見表 | `git-cheatsheet.md` | 講座内で使うGitコマンドのチートシート |

### Sec 03: 作り方の土台（GEMINI.md）

| 配布物 | ファイル | 説明 |
|--------|---------|------|
| GEMINI.md最小テンプレート | `claude-md-template.md` | GEMINI.mdの最小構成テンプレート |
| 曖昧表現辞書 | `ambiguous-expression-dictionary.md` | 「いい感じに」等の曖昧表現→具体化変換リスト |

**撮影用デモファイル** (`filming/staged/`):
- `claude-md-v0.md` ~ `claude-md-v3.md` — デモで段階的に進化させるGEMINI.mdの4バージョン
...
| 撮影用デモファイル（`filming/staged/`） | 9（GEMINI.md 4版 + Skill 5つ） |
| **合計** | 34 |
